...
impl ACCESS_ALLOWED_OBJECT_ACE{
    fn new(bin: Vec<u8>) -> Self{
        ...
        let mut rdr:Cursor<Vec<u8>> = Cursor::new(bin.clone());
        rdr.seek(SeekFrom::Start(64)); // skip header + access mask
...
// TODO: add cli and search piped data for ldapsearches nTSecurityDescriptor:, ObjectSid:, ect attributes and parse them
fn main() {
    ...
    println!("{:X?}",sd.Dacl.as_ref().unwrap().ACEList[0].as_any().downcast_ref::<ACCESS_ALLOWED_OBJECT_ACE>().unwrap().Mask.Mask);
}
